...
StateManager.prototype.initialize = function(callback) {
  console.log("StateManager:initialize");
  var self = this;
  ...
  // console.log("StateManager:initialize : post unlocked_accounts");
  ...
  console.log("StateManager:initialize : pre-blockchain initialize");
  self.blockchain.initialize(accounts, function(err) {
    console.log("StateManager:initialize : post-blockchain initialize");
...
StateManager.prototype.createAccount = function(opts, i) {
  ...
  var account = Account.fromAccountData({
    balance: to.hex(opts.balance)
  });
...
StateManager.prototype.processBlock = function(timestamp, callback) {
  ...
  self.blockchain.processNextBlock(timestamp, function(runtimeError, transactions, vmOutput) {
    ...
    console.log('StateManager.processBlock : processNextBlock' + u.inspect(transactions) )
    async.eachSeries(
      transactions,
...
StateManager.prototype.processTransaction = function(from, tx, callback) {
  ...
  console.log('StateManager.processTransaction : pre createTransactionWithCorrectNonce')
  self.createTransactionWithCorrectNonce(tx, from, function(err, tx) {
    ...
    console.log('StateManager.processTransaction : post queueTransaction')
    ...
    console.log('StateManager.processTransaction : pre processBlocks')

    self.processBlocks(function(err) {
      console.log('StateManager.processTransaction : post processBlocks')
