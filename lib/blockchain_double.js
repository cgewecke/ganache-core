...
BlockchainDouble.prototype.initialize = function(accounts, callback) {
  var self = this;
  console.log("BlockchainDouble:init  pre-data.initialize");
  this.data.initialize(function(err) {
    if (err) {
      return callback(err);
    }

    // console.log("BlockchainDouble:init  pre-self.latestBlock");
    self.latestBlock(function(err, block) {
      // console.log("BlockchainDouble:init  post-self.latestBlock");
      ...
      console.log("BlockchainDouble:init  pre-createStateTrie");
      ...
      console.log("BlockchainDouble:init  post-createVMFromStateTrie");
      ...
      self.createGenesisBlock(async function(err, block) {
        console.log("BlockchainDouble:init  post-createGenesisBlock");
        ...
        await self.vm.stateManager.checkpoint();
        console.log("BlockchainDouble:init  post-checkpoint");

        for (const accountData of accounts) {
          await self.vm.stateManager.putAccount(
            Address.fromString(accountData.address),
            accountData.account
          );
        }

        console.log("BlockchainDouble:init  pre-commit");
        try {
          await self.vm.stateManager.commit();
        } catch (err) {
          console.log("BlockchainDouble: commit err --> " + err);
        }
        console.log("BlockchainDouble:init  pre-self.putBlock");
        self.putBlock(block, [], [], callback);
        console.log("BlockchainDouble:init  post-self.putBlock");
      });
    });
  });
};
...
BlockchainDouble.prototype.latestBlock = async function(callback) {
  let last;
  try {
    // console.log("BlockchainDouble:latestBlock: pre-last()");
    last = await this.data.blocks.last();
  } catch (err) {
    callback(err);
  }
  callback(null, last);
...
BlockchainDouble.prototype.getBlock = async function(number, callback) {
  ...
  if (number.indexOf("0x") >= 0) {
    ...
    if (hash.length > 40) {
      let blockIndex;
      try {
        blockIndex = await this.data.blockHashes.get(to.hex(hash));
      } catch (err) {
        return callback(err);
      }
      ...
      let blocks;
      try {
        blocks = await self.data.blocks.get(blockIndex);
      } catch (err) {
        return callback(err);
      }
      ...
      let blocks;
      try {
        blocks = await self.data.blocks.get(to.number(hash));
      } catch (err) {
        return callback(err);
      }
      return callback(null, blocks);
    ...
    if (number === "latest" || number === "pending") {
      ...
      let first;
      try {
        first = await this.data.blocks.first();
      } catch (err) {
        return callback(err);
      }
...
BlockchainDouble.prototype.putBlock = async function(block, logs, receipts, callback) {
  var self = this;
  ...
  let length;
  try {
    length = await this.data.blocks.length();
  } catch (err) {
    return callback(err);
  }
  ...
  try {
    await self.data.blocks.push(block)
    await self.data.blockLogs.push(logs)
    await self.data.blockHashes.set(to.hex(block.hash()), length)

    for (tx of blockTransactions){
      var txHash = to.hex(tx.hash());
      await self.data.transactions.set(txHash, tx),
      await self.data.transactionReceipts.set(txHash, receipts[index])
    }
    self.emit("block", block);
  } catch (err) {
    e = err
...
BlockchainDouble.prototype.popBlock = async function(callback) {
  var self = this;

  let block;
  try {
    block = await this.data.blocks.last();
  } catch (err) {
    return callback(err);
  }
  ...
  let newLastBlock;
  try {
    newLastBlock = await self.data.blocks.last();
  } catch (err) {
    return callback(err);
  }
...
BlockchainDouble.prototype.createBlock = function(parent, emulateParent, callback) {
  ...
  getParent(function(err, parent) {
    ...
    const headerData = {};
    headerData.gasLimit = self.blockGasLimit;

    if (parent != null && emulateParent) {
      headerData.number = parent.header.number;
      headerData.timestamp = parent.header.timestamp;
      headerData.parentHash = parent.header.parentHash;
    } else {
      var parentNumber = parent != null ? to.number(parent.header.number) : -1;

      // Ensure we have the right block number for the VM.
      headerData.number = to.hex(parentNumber + 1);

      // Set the timestamp before processing txs
      headerData.timestamp = to.hex(self.currentTime());

      if (parent != null) {
        headerData.parentHash = to.hex(parent.hash());
      }
    }
    block = Block.fromBlockData({
      headerData: BlockHeader.fromHeaderData(headerData)
    });
...
BlockchainDouble.prototype.getQueuedNonce = async function(address, callback) {
  var nonce = null;
  ...
  let val;
  try {
    val = await this.stateTrie.get(addressBuffer);
  } catch (err) {
    return callback(err);
  }

  let account, nonceBuffer;
  if (val !== null) {
    account = Account.fromRlpSerializedAccount(val);
  }

  const useZeroBN = val === null
  callback(null, useZeroBN ? new BN(0) : account.nonce);
};
...
BlockchainDouble.prototype.processBlock = async function(vm, block, commit, callback) {
  console.log("top of process block");
  var self = this;
  ...
  block = Block.fromBlockData({
    headerData: BlockHeader.fromHeaderData(block.header),
    transactions: block.transactions,
    uncleHeaders: block.uncleHeaders
  });

  console.log('BlockchainDouble:processBlock : runBlock')
  ...
  const promises = [];
  // const putInTrie = (trie, key, val) => trie.put.call(trie, key, val);
  ...
  for (var v = 0; v < results.receipts.length; v++) {
    ...
    console.log("pre txTriePut");
    await txTrie.put(key, tx.serialize());
    console.log("post txTrie put");
    await rcptTrie.put(key, rcptBuffer);
    console.log("post rcptTrie put");
  }
...
BlockchainDouble.prototype.processNextBlock = function(timestamp, callback) {
  ...
  this.createBlock(async function(err, block) {
    if (err) {
      return callback(err);
    }
    ...
    if (timestamp) {
      let last;
      try {
        last = await self.data.blocks.last();
      } catch (err) {
    ...
    console.log('BlockchainDouble:processNextBlock : pre processBlock')
...
BlockchainDouble.prototype.getAccount = function(address, number, callback) {
  // console.log("BlockchainDouble:getAccount");
  var self = this;

  this.getBlock(number, async function(err, block) {
    // console.log("BlockchainDouble:getAccount : post getBlock");
    if (err) {
      return callback(err);
    }
    ...
    let data;
    // console.log("BlockchainDouble:getAccount : pre stateTrie.get");
    try {
      data = await self.stateTrie.get(utils.toBuffer(address));
      trie.root = currentStateRoot;
    } catch (err) {
      trie.root = currentStateRoot;
      return callback(err);
    }
    console.log("BlockchainDouble:getAccount : pre fromRlpSerializedAccount");
    var account = Account.fromRlpSerializedAccount(data);
    console.log("BlockchainDouble:getAccount : post fromRlpSerializedAccount");
...
BlockchainDouble.prototype.getStorage = function(address, position, number, callback) {
  ...
  this.getBlock(number, async function(err, block) {
    ...
    let data;
    try {
      data = await trie.get(utils.toBuffer(address));
      trie.root = currentStateRoot;
    } catch (err) {
      trie.root = currentStateRoot;
      return callback(err);
    ...
    console.log("BlockchainDouble:getStorage : pre Account.fromRlp");
    var account = Account.fromRlpSerializedAccount(data);
    console.log("BlockchainDouble:getStorage : post Account.fromRlp");

    trie.root = account.stateRoot;

    let value;
    try {
      await trie.get(utils.setLengthLeft(utils.toBuffer(position), 32));
...
BlockchainDouble.prototype.getCode = function(address, number, callback) {
  ...
  this.getBlock(number, async function(err, block) {
    ...
    console.log("BlockchainDouble:getCode : pre Account.fromRlp");
    account.getCode(trie, function(err, code) {
      console.log("BlockchainDouble:getCode : post Account.fromRlp");
...
BlockchainDouble.prototype.getTransaction = async function(hash, callback) {
  hash = to.hex(hash);

  let tx;
  try {
    tx = this.data.transactions.get(hash);
  } catch (err) {
    if (err) {
      if (err.notFound) {
        return callback(null, null);
...
BlockchainDouble.prototype.getTransactionReceipt = async function(hash, callback) {
  hash = to.hex(hash);
  ...
  let receipt;
  try {
    receipt = await this.data.transactionReceipts.get(hash);
  } catch (err) {
    if (err) {
      if (err.notFound) {
        return callback(null, null);
...
BlockchainDouble.prototype.getBlockLogs = function(number, callback) {
  var self = this;
  this.getEffectiveBlockNumber(number, async function(err, effective) {
    if (err) {
      return callback(err);
    }
    let val;
    try {
      val = await self.data.blockLogs.get(effective);
    } catch (err) {
      return callback(err);
    }
...
BlockchainDouble.prototype.getHeight = async function(callback) {
  let length;
  try {
    length = await this.data.blocks.length();
  } catch (err) {
